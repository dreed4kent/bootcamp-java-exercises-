...
CREATE TABLE address (
        ...
        constraint chk_add_other_description check (
                ...
                (address_type <> 'Other' AND address_other_description is not null))
);
...
CREATE TABLE phone (
        ...
        constraint chk_phone_other_description check (
                ...
                (phone_type <> 'Other' AND phone_other_description is not null))
);
...
CREATE TABLE email (
        ...
        email_number varchar(20) not null,
        ...
        constraint chk_emai_type check (email_type IN ('Home', 'Work', 'Other')),
        constraint chk_email_other_description check (
                (email_type = 'Other' AND email_other_description is not null) OR
                (email_type <> 'Other' AND email_other_description is not null))
);
